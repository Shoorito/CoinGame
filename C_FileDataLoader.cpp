...
void C_DataLoader::presetData()
{
	...
	for (int nDataType((int)E_FILE_TYPE::E_COIN); nDataType < (int)E_FILE_TYPE::E_MAX; nDataType++)
	{
		...
		{
			detectSyntax(arData[nDataType][nDataNum], nDataType);
		}
	}
}
...
int C_DataLoader::detectSyntax(const std::string & strSyntax, const int nType)
{
	int	nMaxLength(0);
	int nMaxSize(0);
	int nDataNum(0);
	int	nPosition(0);
	std::string strResult("");
	std::string arResult[(int)E_DATA::E_MAX]{};
	std::string* pResult(nullptr);

	nMaxLength = strSyntax.size() - 1;
	nDataNum = (int)E_DATA::E_FIRST;
	...
	while ((nPosition < nMaxLength - 1) && (nDataNum < (int)E_DATA::E_MAX))
	{
		strResult = getReadText(strSyntax, nPosition, nMaxLength);

		if (strResult != "")
		{
			arResult[nDataNum] = strResult;
		}
